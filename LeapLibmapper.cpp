...
void SampleListener::onInit(const Controller& controller) {
    ...
    controller.setPolicyFlags(Leap::Controller::POLICY_BACKGROUND_FRAMES);
        ...
        myMapper = new mapper::Device("LeapMotion");
        if (myMapper) {
            std::cout<<"Leapmotion mapper device created"<<std::endl;
            while (!myMapper->ready()) {
                myMapper->poll(500);
...
void SampleListener::onExit(const Controller& controller) {
    std::cout << "Exited" << std::endl;
        if (myMapper != NULL) {
            std::cout<<"deallocating mapper"<<std::endl;
            delete myMapper;
        }
}
